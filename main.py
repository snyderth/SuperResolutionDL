...
if __name__ == '__main__':
    ...
    model = Net1(upscale_factor=opt.upscale_factor).to(device)
    ...
    # model = torch.load('net1-baseline/model_epoch_50.pth')
    # model = model.to(device)
    # model.load_state_dict(torch.load('model_epoch_50.pth'))
    # model.to(device)

    # for epoch in range(1, opt.nEpochs + 1):
    #     train(epoch + opt.nEpochs)
    #     test()
    #     checkpoint(epoch + opt.nEpochs)
